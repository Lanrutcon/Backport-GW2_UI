...
function createUnitFrame(unitWatch,relativePoint)
	...
	local altPowerHolderC = CreateFrame("frame",'altPowerHolderC', ComboFrame)
	altPowerHolderC:SetFrameStrata("BACKGROUND")
	altPowerHolderC:SetWidth(325)
	altPowerHolderC:SetHeight(15)
	altPowerHolderC:Show()
    ...
    b:SetScript("OnEvent",function(self,event,unit)
        if not UnitExists(unitWatch) then
            unitFrameTargetTargetAnimation[unitWatch] =0
            TTbarBG:SetAlpha(0)
            TTarrow:SetAlpha(0)
        ...
        --local absorbAmount = UnitGetTotalAbsorbs(unitWatch) / UnitHealthMax(unitWatch)  --doesn't exist
