...
local function initBars()
		...
		ShapeshiftBarFrame:SetSize(100, 32)
        ShapeshiftButton1:SetPoint('CENTER', UIParent, 'CENTER', 0, 0)
        ShapeshiftBarFrame:SetPoint('CENTER', UIParent, 'CENTER', -15, 67)
        ...
        PVPMicroButton:SetScale(0.6)
        EJMicroButton:SetScale(0.6)
        RaidMicroButton:SetScale(0.6)
        MainMenuMicroButton:SetScale(0.6)
        HelpMicroButton:SetScale(0.6)
        ...
        function setMoveableActionbars()
			...
			name = "frameMultiBarLeftdrageAbleFrame";
            ...
            frameMultiBarLeftdrageAbleFrame:SetPoint(GW2UI_SETTINGS[name]['point'],GW2UI_SETTINGS[name]['xOfs'],GW2UI_SETTINGS[name]['yOfs'])
			...
			name = "frameMultiBarRightdrageAbleFrame";
            ...
            frameMultiBarRightdrageAbleFrame:SetPoint(GW2UI_SETTINGS[name]['point'],GW2UI_SETTINGS[name]['xOfs'],GW2UI_SETTINGS[name]['yOfs'])
            frameMultiBarRightdrageAbleFrame:SetFrameStrata('HIGH');
			
			--ShapeshiftBarFrame
			local frameShapeshiftdrageAbleFrame = CreateFrame("Frame","frameShapeshiftdrageAbleFrame",UIParent)
            frameShapeshiftdrageAbleFrame:SetWidth(ShapeshiftBarFrame:GetWidth())
            frameShapeshiftdrageAbleFrame:SetHeight(ShapeshiftBarFrame:GetHeight())
            frameShapeshiftdrageAbleFrame.texture = frameShapeshiftdrageAbleFrame:CreateTexture()

			name = "frameShapeshiftdrageAbleFrame";
            frameShapeshiftdrageAbleFrame.texture:SetAllPoints(frameShapeshiftdrageAbleFrame)
            frameShapeshiftdrageAbleFrame.texture:SetTexture(1,0,0,0.5)
            frameShapeshiftdrageAbleFrame:SetPoint(GW2UI_SETTINGS[name]['point'],GW2UI_SETTINGS[name]['xOfs'],GW2UI_SETTINGS[name]['yOfs'])
            frameShapeshiftdrageAbleFrame:SetFrameStrata('HIGH');

			--PetActionBarFrame
			local framePetActionBardrageAbleFrame = CreateFrame("Frame","framePetActionBardrageAbleFrame",UIParent)
            framePetActionBardrageAbleFrame:SetWidth(PetActionBarFrame:GetWidth())
            framePetActionBardrageAbleFrame:SetHeight(PetActionBarFrame:GetHeight())
            framePetActionBardrageAbleFrame.texture = framePetActionBardrageAbleFrame:CreateTexture()

			name = "framePetActionBardrageAbleFrame";
            framePetActionBardrageAbleFrame.texture:SetAllPoints(framePetActionBardrageAbleFrame)
            framePetActionBardrageAbleFrame.texture:SetTexture(1,0,0,0.5)
            framePetActionBardrageAbleFrame:SetPoint(GW2UI_SETTINGS[name]['point'],GW2UI_SETTINGS[name]['xOfs'],GW2UI_SETTINGS[name]['yOfs'])
            framePetActionBardrageAbleFrame:SetFrameStrata('HIGH');
			...
			ShapeshiftBarFrame:SetPoint('CENTER',frameShapeshiftdrageAbleFrame,'CENTER',0,0)
			PetActionButton1:SetPoint('CENTER',framePetActionBardrageAbleFrame,'CENTER',0,0)
			
			--CHANGES:Lanrutcon:OnDragStop function
			local function onDragStop(self)
				self:StopMovingOrSizing()
                local point, relativeTo, relativePoint, xOfs, yOfs = self:GetPoint()
                GW2UI_SETTINGS[self:GetName()]['point'] = point
                GW2UI_SETTINGS[self:GetName()]['relativePoint'] = relativePoint
                GW2UI_SETTINGS[self:GetName()]['xOfs'] = xOfs
                GW2UI_SETTINGS[self:GetName()]['yOfs'] = yOfs
			end
			
			local movableFrames = { frameMultiBarLeftdrageAbleFrame, frameMultiBarRightdrageAbleFrame, frameShapeshiftdrageAbleFrame, framePetActionBardrageAbleFrame };
			for numFrame = 1, #movableFrames do
				movableFrames[numFrame]:SetMovable(false);
				movableFrames[numFrame]:EnableMouse(false);
				movableFrames[numFrame]:RegisterForDrag("LeftButton")
				movableFrames[numFrame]:SetScript("OnDragStart", movableFrames[numFrame].StartMoving)
				movableFrames[numFrame]:SetScript("OnDragStop", onDragStop);
